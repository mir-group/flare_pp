...
B2_Spec ::B2_Spec(const std::string &radial_basis, const std::string &cutoff_function,
...
void B2_Spec ::write_to_file(std::ofstream &coeff_file, int coeff_size) {
...
DescriptorValues B2_Spec ::compute_struc(Structure &structure) {
...
#pragma omp parallel for
  for (int atom = 0; atom < n_atoms; atom++) {
    ...
    int counter = 0;
    for (int s1 = 0; s1 < nos; s1++) {
      for (int n1 = 0; n1 < N; n1++) {
        for (int n2 = n1; n2 < N; n2++) {
          for (int l = 0; l < (lmax + 1); l++) {
            for (int s2 = 0; s2 < nos; s2++) {
              for (int m = 0; m < (2 * l + 1); m++) {
                n1_l = ((s1 * N) + n1) * n_harmonics + (l * l + m);
                n2_l = ((s2 * N) + n2) * n_harmonics + (l * l + m);
