...
def test_update_db():
    ...
    sgp_py.update_db(train_structure, forces, [3], energy, stress, mode="uncertain")
...
def test_lammps():
    ...
    new_kern = sgp_py.write_varmap_coefficients(
        "beta_var.txt", "B", 0
    ...
    for s1 in range(n_species):
        ...
        beta_matr = np.reshape(
            sgp_py.sgp_var.varmap_coeffs[s1, :], (n_descriptors, n_descriptors)
        )
